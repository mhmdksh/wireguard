...
PersistentKeepalive = 15" >>"${HOME_DIR}/${SERVER_WG_NIC}-client-${CLIENT_NAME}.conf"
...
AllowedIPs = ${CLIENT_WG_IPV4}/32,${CLIENT_WG_IPV6}/128" >>"/etc/wireguard/${SERVER_WG_NIC}.conf"
	...
	qrencode -t ansiutf8 -l L <"${HOME_DIR}/${SERVER_WG_NIC}-client-${CLIENT_NAME}.conf" > "${HOME_DIR}/${SERVER_WG_NIC}-qr-client-${CLIENT_NAME}.txt"
	qrencode -t ansiutf8 -l L <"${HOME_DIR}/${SERVER_WG_NIC}-client-${CLIENT_NAME}.conf"

	echo "Config file available in ${HOME_DIR}/${SERVER_WG_NIC}-client-${CLIENT_NAME}.conf"
	echo "QR Code file is also available in ${HOME_DIR}/${SERVER_WG_NIC}-qr-client-${CLIENT_NAME}.txt"
}
...
function revokeClient() {
	...
	rm -f "${HOME}/${WorkingDirPrefix}/${SERVER_WG_NIC}-client-${CLIENT_NAME}.conf"
	rm -f "${HOME}/${WorkingDirPrefix}/${SERVER_WG_NIC}-qr-client-${CLIENT_NAME}.txt"
